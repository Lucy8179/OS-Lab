...
int createTsk(void (*tskBody)(void)){//在进程池中创建一个进程，并把该进程加入到rqFCFS队列中
	if(firstFreeTsk==NULL){
		...
		myTCB *new_task=firstFreeTsk;
		firstFreeTsk=new_task->nextTCB;
		new_task->task_entrance=tskBody;
		stack_init(&new_task->stkTop,tskBody);
		tskStart(new_task);
		return new_task->TSK_ID;
	}
}

//以takIndex为关键字，在进程池中寻找并销毁takIndex对应的进程（需要填写）
void destroyTsk(int takIndex) {//在进程中寻找TSK_ID为takIndex的进程，并销毁该进程
	tcbPool[takIndex].TSK_State=TSK_WAIT;
	tcbPool[takIndex].nextTCB=NULL;
	tcbPool[takIndex].task_entrance=NULL;
	return;
}
